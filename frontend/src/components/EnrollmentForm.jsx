...
export default function EnrollmentForm({ coordinatorId, onSubmitted }) {
  ...
  const calculateAge = (dob) => {
    if (!dob) return '';
    const [day, month, year] = String(dob).split('-');
  ...
  const handleChange = (e) => {
    ...
    if (name === 'voter_id' || name === 'acknowledgement_number' || name === 'booth_number') value = value.replace(/[^a-z0-9]/gi, '').toUpperCase();
    if (name === 'email') value = value.toLowerCase();
    if (name === 'pincode') value = value.replace(/\D/g, '').slice(0, 6);
    if (name === 'date_of_birth') value = formatDateInput(value);
  ...
  const validateField = (name, value) => {
    ...
    if (name === 'booth_number' && value && !/^[A-Z0-9]+$/.test(value)) return 'Enter a valid ward number using uppercase letters and numbers only.';
    if (name === 'date_of_birth' && value && !/^\d{2}-\d{2}-\d{4}$/.test(value)) return 'Enter date as DD-MM-YYYY.';
  ...
  const handleFileChange = (e) => {
    ...
    const mergedFiles = [...files, ...selectedFiles].filter((file, index, collection) => collection.findIndex(item => item.name === file.name && item.size === file.size && item.lastModified === file.lastModified) === index);
    ...
    <div className="mx-auto w-full p-4 sm:p-6 lg:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 pb-8">
        ...
        <section className="space-y-3 rounded-lg border border-[#e4ebe7] bg-[#f7faf8] p-4"><h3 className="border-b border-[#e4ebe7] pb-2 text-sm font-bold uppercase tracking-wider text-[#52736a]">Address and constituency</h3>
          ...
          <div className="grid gap-3 sm:grid-cols-2"><Field label={<><span>Ward Number</span>{requiredStar}</>} name="booth_number" value={formData.booth_number} onChange={handleChange} onBlur={handleBlur} error={fieldErrors.booth_number} placeholder="Uppercase letters/numbers" /><div><label className={labelClass}><span>Mandal</span>{requiredStar}</label><select name="mandal" required value={formData.mandal} onChange={handleChange} disabled={!formData.constituency} className={inputClass}><option value="">Select mandal</option>{mandals.map(item => <option key={item.mandal} value={item.mandal}>{item.mandal}</option>)}</select></div></div>
        ...
        <section className="space-y-3 rounded-lg border border-[#e4ebe7] bg-[#f7faf8] p-4"><h3 className="border-b border-[#e4ebe7] pb-2 text-sm font-bold uppercase tracking-wider text-[#52736a]">Supporting documents</h3><label className={labelClass}><span>Supporting documents</span><span className="ml-1 font-normal text-[#849890]">(optional)</span></label><div className="relative rounded-md border-2 border-dashed border-[#b6cbc3] bg-white p-5 text-center hover:bg-[#f2f8f4]"><input type="file" multiple onChange={handleFileChange} className="absolute inset-0 h-full w-full cursor-pointer opacity-0" accept=".pdf,.jpg,.jpeg,.png" /><UploadCloud className="mx-auto mb-2 text-[#1d6b5d]" size={24} /><span className="text-sm font-medium text-[#465b55]">{files.length ? 'Add another document' : 'Choose supporting documents (optional)'}</span><p className="mt-1 text-xs text-[#849890]">Select up to 2 files. Accepted types: PDF, JPG, PNG</p></div>{files.length > 0 && <div className="space-y-2">{files.map(file => <div key={`${file.name}-${file.size}-${file.lastModified}`} className="flex items-center justify-between rounded-md border border-[#d8e5df] bg-white px-3 py-2 text-sm text-[#465b55]"><span className="truncate pr-3">{file.name}</span><button type="button" onClick={() => removeFile(file)} className="rounded p-1 text-[#b44d45] hover:bg-[#fff3f1]" aria-label={`Remove ${file.name}`} title="Remove document"><Trash2 size={16} /></button></div>)}</div>}</section>
