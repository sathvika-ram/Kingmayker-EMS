...
import { BarChart3, CheckCircle, Clock3, Download, FileSpreadsheet, LayoutDashboard, LogOut, Map, Menu, RefreshCw, Search, Users, X } from 'lucide-react';
...
export default function LeaderDashboard() {
  ...
  const today = getTodayCount(voters);
    ...
    <div className="flex w-full">
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} fixed inset-y-0 left-0 z-40 mt-[65px] w-72 border-r border-[#dce8e2] bg-[#173b35] p-4 text-white transition-transform lg:sticky lg:top-[65px] lg:mt-0 lg:h-[calc(100vh-65px)] lg:w-64 lg:translate-x-0`}>
        ...
        <nav className="space-y-1">
          ...
          <SideLink icon={<Map size={17} />} label="Geographic performance" active={activeView === 'geography'} onClick={() => navigate('geography')} />
          <SideLink icon={<FileSpreadsheet size={17} />} label="Enrollment records" active={activeView === 'records'} onClick={() => navigate('records')} />
        </nav>
      </aside>
