...
export default function SuperAdminDashboard() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [summary, feed, coordinatorResult, geographyResult, auditResult, regionResult] = await Promise.all([
        axios.get(`${API}/admin/summary`, { params: requestParams }),
        axios.get(`${API}/admin/enrollments`, { params: { ...requestParams, page, limit: 50 } }),
      ...
      setOverview(summary.data); setVoters(feed.data.voters || []); setFeedPagination(feed.data.pagination || {}); setCoordinators(coordinatorResult.data.coordinators || []); setGeography(geographyResult.data.geography || []); setLogs(auditResult.data.logs || []); setRegions(regionResult.data || []); setError('');
    } catch (err) { setError(err.response?.data?.error || 'Unable to connect to the admin control center.'); }
    finally { setLoading(false); }
  };

  useEffect(() => { load(); const timer = setInterval(load, 15000); return () => clearInterval(timer); }, [filters.region, filters.constituency, filters.mandal, filters.status, filters.search, geoSearch, page]);
  useEffect(() => { setPage(1); }, [filters.region, filters.constituency, filters.mandal, filters.status, filters.search]);
  ...
  return <div data-admin-view={active} className="min-h-screen bg-[#f4f7f8] text-slate-900">
    ...
    <div className="flex">
      ...
      <main className="min-w-0 flex-1 p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-[1500px] space-y-8"><Pagination page={page} totalPages={feedPagination.total_pages} onPageChange={setPage} />{error && <div className="flex items-center justify-between rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700"><span>{error}</span><button onClick={() => setError('')} aria-label="Dismiss error"><X size={16} /></button></div>}
...
function InlineCoordinatorForm({ onCreated }) {
  ...
  return (
    <div className="mb-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
      <div className="mb-4">
        <h3 className="text-base font-bold text-slate-900">Add coordinator</h3>
        <p className="mt-1 text-xs text-slate-500">Create a new constituency access account.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-slate-700">Temporary password <span className="text-red-500">*</span></label>
          <input type="text" name="temp_password" required value={formData.temp_password} onChange={handleChange} className={inputClass} placeholder="e.g. Temp@123" />
        </div>
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-slate-700">Agent personal email ID</label>
          <input type="email" name="agent_personal_email" value={formData.agent_personal_email} onChange={handleChange} className={inputClass} placeholder="agent@example.com" />
        </div>

        <div>
          <label className="mb-1 block text-sm font-medium text-slate-700">Assigned mandal</label>
          <select name="assigned_mandal" value={formData.assigned_mandal} onChange={handleChange} disabled={!formData.assigned_constituency} className={inputClass}>
        ...
        <div className="md:col-span-2 flex justify-end">
          <button type="submit" disabled={loading} className="rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-800 disabled:cursor-not-allowed disabled:bg-teal-400 text-white">
...
function Pagination({ page, totalPages = 1, onPageChange }) { return <div className="flex items-center justify-end gap-2 py-3 text-xs font-semibold text-slate-500"><button type="button" disabled={page <= 1} onClick={() => onPageChange(page - 1)} className="rounded border border-slate-300 bg-white px-3 py-1.5 disabled:opacity-40">Previous</button><span>Page {page} of {Math.max(totalPages, 1)}</span><button type="button" disabled={page >= totalPages} onClick={() => onPageChange(page + 1)} className="rounded border border-slate-300 bg-white px-3 py-1.5 disabled:opacity-40">Next</button></div>; }
