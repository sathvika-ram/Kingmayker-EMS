...
function getRegionGrowth(voters, selectedRegion, regionOptions = []) {
  ...
  return regionNames.map(region => ({
    ...
    values: labels.map((_, index) => {
      ...
      return voters.filter(voter => String(voter.region || '').trim() === String(region || '').trim() && new Date(voter.created_at).toISOString().slice(0, 10) === key).length;
    })
  }));
}
